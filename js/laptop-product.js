import { laptop, laptop_hero } from "./data.js";
...
for (let i = 0; i < 8; i++) {
  ...
  display_arrival.innerHTML += `
   <div class="col-6 col-lg-3" id="what" onclick="get_item(${laptop[random_index].id})">
            <div class="product">
              <div class="img-wrap">
                <img
                  src="${laptop[random_index].image}"
                  alt=""
                />
              </div>
              <div class="cat">id:${laptop[random_index].id}</div>
              <div class="name">${laptop[random_index].short_name}</div>
              <div class="price text-cyan">${laptop[random_index].price}$</div>
            </div>
          </div>
`;
}
//laptop hero
const change_hero = document.getElementById("laptop_product");
let currentIndex = 0;
change_hero.innerHTML = `<div class="col-lg-6" >
            <span class="chip mb-4"
              ><span class="dot"></span> Flagship Mobility</span
            >
            <h1 class="mt-3 text-gradient">
            ${laptop_hero[currentIndex].name}<span class="ghost">${laptop_hero[currentIndex].marketing}</span>
            </h1>
            <p class="text-muted2 mt-3" style="max-width: 460px">
              ${laptop_hero[currentIndex].description}
            </p>

            <div class="row g-2 mt-3" style="max-width: 460px">
              <div class="col-6">
                <div class="spec-pill">
                  <span class="lbl">CPU</span><span>${laptop_hero[currentIndex].spec.cpu}</span>
                </div>
              </div>
              <div class="col-6">
                <div class="spec-pill">
                  <span class="lbl">GPU</span><span>${laptop_hero[currentIndex].spec.gpu}</span>
                </div>
              </div>
              <div class="col-6">
                <div class="spec-pill">
                  <span class="lbl">RAM</span><span>${laptop_hero[currentIndex].spec.ram}DDR5</span>
                </div>
              </div>
              <div class="col-6">
                <div class="spec-pill">
                  <span class="lbl">Display</span><span>${laptop_hero[currentIndex].spec.display}</span>
                </div>
              </div>
            </div>

            <div class="d-flex gap-3 mt-4">
              <button class="btn btn-cyan" onclick="add_to_cart(${laptop_hero[currentIndex].id})">Add to Cart</button>
              <button class="btn btn-ghost" onclick="get_item(${laptop_hero[currentIndex].id})"> View Specs</button>
            </div>
          </div>
          <div class="col-lg-6 text-center">
            <img
              src="${laptop_hero[currentIndex].image}"
              alt="Featured laptop"
              class="laptop-img"
...
setInterval(() => {
  ...
  setTimeout(() => {
    currentIndex = (currentIndex + 1) % laptop_hero.length;
    change_hero.innerHTML = `<div class="col-lg-6" >
            <span class="chip mb-4"
              ><span class="dot"></span> Flagship Mobility</span
            >
